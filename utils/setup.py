...
from utils.JsonSocket import JsonSocket
...
def generatePrivateKey(output_path: str):
  try:
...
def generateAndSignCertificate(logger: logging.Logger, host: str, private_key: rsa.RSAPrivateKey, cert_path: str, ca_path):
  cert_subject = x509.Name(
    [x509.NameAttribute(NameOID.COMMON_NAME, "Savant-Lutron-QSX-Bridge")]
  )
  ...
  with socket.create_connection((host, 8083)) as raw_socket:
    ...
    logger.info("Press and release the small black unlabeled button on the homeworks bridge bridge to allow for certificate approval...")
    ...
    logger.info("Asking QSX processor to sign the csr for us")
    ...
    sock.write_json(
        {
            ...
            "Body": {
                ...
                "Parameters": {
                    "CSR": csr_text,
                    "DisplayName": "Savant-Lutron-QSX-Bridge",
    ...
    while True:
      ...
      logger.info("Got certificates from QSX processor")
      tls_socket.shutdown()

def pingLeapHost(host: str, key_path: str, cert_path: str, ca_path:str):
  ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLSv1_2)
  ssl_context.load_verify_locations(ca_path)
  ssl_context.load_cert_chain(cert_path, key_path)
  # TODO: fix certificate chain validation
  ssl_context.verify_mode = ssl.CERT_NONE

  with socket.create_connection((host, 8081)) as raw_socket:
